...
export function FeaturedPGs() {
  ...
  const fetchPGs = async () => {
    try {
      ...
      // REMOVED MOCK DATA - Only use real data from API
      if (pgs.length === 0) {
        // Don't show mock data, just set empty array
        console.log('No PGs found in database');
        setAllPGs([]);
        setDisplayedPGs([]);
        setError('No PG listings found');
        return;
      }
      
      // Filter out any PGs without images or with invalid data if needed
      // But keep all PGs - don't filter by images
      setAllPGs(pgs);
      updateDisplayedPGs(pgs);
      ...
      // IMPORTANT: DON'T show mock data on error
      // Set empty arrays instead
      setAllPGs([]);
      setDisplayedPGs([]);
    } finally {
      setLoading(false);
